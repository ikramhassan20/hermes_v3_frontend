<template>
    ...
    vertical-align: middle;" class="apply_btn" type="button" v-on:click="exportUsers($route.params.id)">Export
              </button>
              <button style="
    display: inline-block;
    vertical-align: middle;margin-left: 5px;" class="apply_btn" type="button" v-on:click="fetchTrackingStats($route.params.id)">Reload Stats
              </button>
        ...
        <div class="state_header no_padding">
            <table class="table table-bordered text-center">
                ...
                <tbody>
                <tr>
                    <td style="text-align: center;" class="col-background-color">{{ trackingStats.targetedUsers }}</td>
        ...
        <div v-if="campaignExpireStatus == 0" class="state_header no_padding details_header new_cal_div">
        ...
        <div v-if="campaignExpireStatus == 0" class="table_holder no_padding details_table">
    ...
    export default {
      ...
      props: ['campaignExpireStatus'],
        ...
        data() {
            ...
            return {
                trackingDataTable: {
                    ...
                    column: ['email', 'variant', 'track_key', 'sent_at', 'status', 'device_type', 'viewed_at', 'message', 'Action'],
                    mapColumns: ['email', 'id', 'row_id', 'variantLang', 'variant', 'track_key', 'sent_at', 'status', 'device_type', 'viewed_at', 'message'],
                    filterable: ['email', 'track_key', 'sent_at', 'status', 'device_type', 'viewed_at', 'message'],
                    sortable: ['email', 'track_key', 'sent_at', 'status', 'device_type', 'viewed_at', 'message'],
                    headings: {
                        email: 'Email ',
                        track_key: 'Track key',
                        sent_at: 'Sent At',
        ...
        methods: {
            ...
            fetchTrackingStats(campaignId) {
                const payLoad = {
                    resource: "campaign/tracking/stats/" + campaignId,
                    action: "get",
                    data: {
