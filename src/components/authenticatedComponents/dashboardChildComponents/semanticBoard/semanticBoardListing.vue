                ...
                <header class="sec_header">
                    ...
                    <div class="align_right">
                        ...
                        <ul class="actions_drop list_none">
                            <li>
                                <router-link to="/dashboard/semantixBoard-create">Semantix Board Creation</router-link>
                            </li>
                        </ul>
                    </div>
                </header>
                <div class="list_holder">
                    ...
                    <div class="table_holder mCustomScrollbar">
                        <listing v-bind:tableColumn="dataTable.column"
                            ...
                            <template slot="listing" slot-scope="data">
                                ...
                                <ul class="table_drop list_none"  v-bind:id="data.actionlisting.row.id + '_action'">
                                    <li v-if="data.actionlisting.row.status == 'draft'">
                                        <router-link
                                                v-bind:to="'/dashboard/board-edit/edit/' + data.actionlisting.row.id"
                                                data-toggle="modal"
                                    ...
                                    <li v-else>
                                        <router-link
                                                v-bind:to="'/dashboard/board-edit/view/' + data.actionlisting.row.id"
                                                data-toggle="modal"
                                                data-target="#add_lookup"><i
                                    ...
                                    <li v-if="data.actionlisting.row.status != 'draft'">
                                        <router-link
                                                v-bind:to="'/dashboard/board-stats/' + data.actionlisting.row.id"><i
                                                class="fas fa-list"></i>View Stats
                                        </router-link>
                                    </li>
                                    <li v-if="data.actionlisting.row.status == 'active'">
                                        <a class="anchor_pointer"
                                           v-on:click="suspendBoard(data.actionlisting.row.id)"
                                           data-toggle="modal"
                            ...
                            <!--                                <i v-if="data.nameColumn.row.board_type == 'inapp'"-->
                            <!--                                   class="fas fa-mobile-alt"></i>-->
                            <!--                                <i v-else-if="data.nameColumn.row.board_type == 'email'"-->
                            <!--                                   class="far fa-envelope"></i>-->
                            <!--                                <i v-else="data.nameColumn.row.board_type == 'push'"-->
...
<script>
    ...
    export default {
        ...
        data() {
            return {
                dataTable: {
                    ...
                    column: ['name', 'status', 'schedule_type', 'delivery_type', 'priority', 'updated_at', 'Action'],
                    mapColumns: ['id', 'name', 'status', 'schedule_type', 'delivery_type', 'priority', 'updated_at'],
                    filterable: ['name', 'schedule_type', 'delivery_type', 'priority', 'updated_at'],
                    sortable: ['name', 'schedule_type', 'delivery_type', 'priority', 'updated_at'],
        ...
        mounted() {
            this.getBoardSideFilters();
        },
        methods: {
            getBoardSideFilters() {
                const payLoad = {
                    resource: "board/filters",
                    action: "get",
                };
                this.api("post",
            ...
            exportUsers(boardId) {
                ...
                this.api("post",
                    ...
                    console.log('exportCsv',response);
                    if (response.data.meta.code === 200) {
                        ...
                        // this.downloadCSV("board - " + new Date().toLocaleDateString() + " " + new Date().toLocaleTimeString() + ".csv", response.data.data);
                    }
                }).catch((error) => {
            ...
            suspendBoard(boardId) {
                this.$swal({
                    title: "Are you sure ?  You want to suspend this Semantix Board !",
                    type: "warning",
